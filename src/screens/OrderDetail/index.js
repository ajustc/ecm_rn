...
const OrderDetail = props => {
  ...
  const navigateToWebview = () => {
    console.log({url: dataOrderList?.order_redirect_url});
    ...
    <View>
      ...
      <View>
        ...
        dataOrderList?.status === 'settlement' ? (
          <Text style={tw`bg-green-100 text-center py-5 font-bold`}>Thank you for purchase in apps.</Text>
        ) : dataOrderList?.order_status === 'paid' &&
          ...
          <Text style={tw`bg-yellow-100 text-center py-5 font-bold`}>Your order has been failed.</Text>
        )}
      </View>
      ...
      {dataOrderList && (
        <>
          <View style={tw`bg-pink-100 py-[18px] px-[12px] shadow`}>
            <Text style={tw`text-lg text-black font-semibold`}>Purchase</Text>
            <Text style={tw`text-base`}>{dataOrderList?.number}</Text>
            <Text style={tw`text-base`}>{dataOrderList?.order_purchase}</Text>
            <Text style={tw`text-base`}>Rp.{dataOrderList?.order_total}</Text>
          </View>

          <View style={tw`bg-pink-100 py-[18px] px-[12px] shadow`}>
            <Text style={tw`text-lg text-black font-semibold`}>Customer</Text>
          ...
          <View style={tw`bg-pink-100 py-[18px] px-[12px] shadow`}>
            <Text style={tw`text-lg text-black font-semibold`}>Delivery</Text>
            <Text style={tw`text-base`}>{dataOrderList?.order_expedition}</Text>
            <Text style={tw`text-base`}>
              Rp.{dataOrderList?.order_shipping_cost}
            </Text>
            <Text style={tw`text-base`}>{dataOrderList?.order_address}</Text>
          </View>
        </>
      )}
      ...
      {dataOrderDetail?.length > 0 ? (
        dataOrderDetail.map(item => {
          ...
          return (
            <View
              ...
              <Text style={tw`text-black`}>Price : Rp{item.product_price}</Text>
        ...
        <Text style={tw`text-red-500 text-[22px] text-center p-5`}>NoData</Text>
      )}
    </View>
  );
};
